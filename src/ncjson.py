...
class DStoJSON():
    ...
    A class to convert most fields in a netCDF file to a JSON format. Arrays
    with (many) floats are omitted, and represented by an empty list.
    ...
    json_output_filename : str
        name of json file for output. If not given, the filename is constructed
        from the url.
    json_output_directory : str
        name of the directory to write the json files in. Only applies if
        json_output_filename is not given.
    write_to_stdout : bool {False}
        if set to True, the information is also output to stdout.

    Ideally this class should be used in a context, taking care of closing the
    netCDF file.

    >>> with DStoJSON(url) as writer:
    >>>     writer.write_json()
    '''
    # we will process arrays with text, possibly contaminated with
    # floats/nans, just to a depth of this value.
    MAXARRAYSIZE = 15

    def __init__(self, url, json_output_filename='',
                 json_output_directory='.'):
        self.json_output_filename = json_output_filename or \
            self.construct_json_output_filename_from_url(url,
                                                         json_output_directory)
    ...
    def __exit__(self, *p, **kwd):
        # I think I don't understand this xarray,
        # because this does not seem to close the
        # file.
        self.ds.close()

    def construct_json_output_filename_from_url(self, url,
                                                json_output_directory):
        ...
        json_output_filename = os.path.join(json_output_directory,
                                            os.path.basename(url)+'.json')
        return json_output_filename

    def write_json(self, write_to_stdout=False):
        ''' Writes JSON file. '''
        json_dict = {"dimensions": self.get_dimensions(),
                     "dimensions_without_coords":
                     self.get_dimensions_without_coords(),
        ...
        for k, v in self.ds.data_vars.items():
            if v.shape == ():  # zero-sized array
                ...
                if dtype == bytes:
                    ...
                    # this could be nan, need to make json.dump aware of this.
                    variables[k] = _v
            else:
                vp = []
    ...
    def _bytes_decoder(self, bs):
        ...
        try:
            ...
            if e.reason == 'invalid start byte' or \
               e.reason == 'invalid continuation byte':
                ...
                # algorithm.
                b = [i for i in bs]  # make list of bytes we can manipulate.
                for i in range(e.start, e.end):
                    b[i] = 63  # 'corresponds to "?"
                s = self._bytes_decoder(bytes(b))  # create a bytestring.
            else:
                # we got an unexpected decode error. Reraise the exception
                # so we know what happened.
                raise e
        return s.strip()  # remove any trailing white spaces.

    def _get_dimensions(self, s):
        dimensions = [k for k in self.ds.dims
                      if bool(self.ds.get(k).coords) == s]
        return dimensions
